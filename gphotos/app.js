...
app.post('/loadFromList', async (req, res) => {
  const authToken = req.user.token;

  winston.info('Loading images from list.');
  winston.info('Received form data: ', req.body);
  winston.info('End of data from request.');
  ...
  // Create the parameters that will be submitted to the Library API.
  const parameters = {filters};
  winston.info('Paramters: ', parameters)

  // Submit the search request to the API and wait for the result.
  const data = await libraryApiDriveList(authToken, parameters);
...
// example:
// GET https://www.googleapis.com/drive/v3/files?orderBy=modifiedTime&pageSize=10&q=mimeType%3D'image%2Fjpeg'+and+trashed%3Dtrue&spaces=drive&key={YOUR_API_KEY}
//
async function libraryApiDriveSearch(authToken, parameters) {
  ...
  try {
    ...
    // and while there is a nextPageToken to load more items.
    do {
      var apiRequestURL = `${config.apiDriveEndpoint}/v3/files?orderBy=modifiedTime&pageSize=100&spaces=drive&fields=files%2CincompleteSearch%2Ckind%2CnextPageToken&key={${authToken}}`;
      winston.info(`Submitting search with parameters: ${JSON.stringify(parameters)}`);
      winston.info(`API Request: ${apiRequestURL}`);

      // Make a POST request to search the library or album
      // GET https://www.googleapis.com/drive/v3/files?orderBy=modifiedTime&pageSize=100&spaces=drive&fields=files%2CincompleteSearch%2Ckind%2CnextPageToken&key={YOUR_API_KEY}

      //const result = await request.post(apiRequestURL);
      const result = await request.get(config.apiEndpoint + '/v1/files', {
        headers: {'Content-Type': 'application/json'},
        qs: parameters,
        json: true,
        auth: {'bearer': authToken},
...
async function libraryApiDriveList(authToken, parameters) {
  ...
  try {
    ...
    do {
      ...
      //const result = await request.post(apiRequestURL);
      const result = await request.get(config.apiEndpoint + '/v3/files', {
        headers: {'Content-Type': 'application/json'},
        qs: parameters,
        json: true,
        auth: {'bearer': authToken},
